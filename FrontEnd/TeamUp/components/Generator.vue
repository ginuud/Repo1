...
const teamNames = reactive(
  Array.from({ length: state.numberOfTeams }, () => "")
);
...
const validate = (state: any): FormError[] => {
  ...
  if (state.selectedPlayers.length < 4) {
    errors.push({
      ...
      message: "Choose at least 4 players",
    });
  }
...
async function onSubmit(event: FormSubmitEvent<any>) {
  ...
  const transformedData = state.selectedPlayers.map((player) => ({
    id: player.value.id,
